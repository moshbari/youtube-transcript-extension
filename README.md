...
- **Drop in a playlist link and every video in it is scraped** — the whole playlist, not just
  the first 100. YouTube's playlist page renders only 100 rows and scrolling loads no more, so
  the extension follows the page's own continuation chain instead (a 534-video playlist comes
  back in about 2 seconds). Works on your own unlisted/private playlists too, since it all runs
  in your own signed-in browser — no YouTube Data API, no key, no quota.
...
3. Click **Load playlist & scrape all**. The playlist opens in a tab, every video in it fills the
   box, and the batch then runs through them one by one.
